...
const flattenData = (
  ...
  let objectWithYCount: number = 0;
  let multiSeriesCount: number = 0;
...
function getMaxValue(data: Array<any>) {
  let values = [];
...
export const getGuideArray = (
  max: string | number,
  height: number,
  numberOfPoints = 5
) => {
  max = typeof max === "string" ? parseInt(max, 0) : max;
  let x = max;
...
export const numberWithCommas = (x: number, summary: boolean = true) => {
  let postfix = "";
  if (summary) {
    if (x >= 1000 && x < 1000000) {
      postfix = "K";
