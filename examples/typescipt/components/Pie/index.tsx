...
class PieChart extends React.Component<PieChartProps, PieChartState> {
  ...
  handleEventOld(evt) {
    ...
    this.test.measure((fx, fy, width, height, px, py) => {
      let evtX = pageX - px;
      let evtY = pageY - py;
      let originX: number = pageX - px - size / 2;
      let originY: number = size / 2 - (pageY - py);
      ...
      let inPie: boolean = rSquare < Math.pow(size / 2, 2);
      let selectedIndex = -1;
      if (inPie) {
  ...
  */
  ...
  drawInfoOld(x, y) {
    ...
    if (dist <= size) {
      ...
      let pos =
        x > 0
          ? Math.PI / 2 - Math.asin(y / dist)
          : (Math.PI * 3) / 2 + Math.asin(y / dist);
      ...
      return (
        ...
        >
          <Text>
            {Math.round((this.state.pieSize[index] / (2 * Math.PI)) * 10000) /
              100 +
              "\n" +
  ...
  drawInfoT(index) {
    ...
    let pos =
      (this.state.oldPiePos[index] +
        this.state.oldPiePos[(index + 1) % this.state.oldPiePos.length]) /
      2;
    ...
    if (index !== -1) {
      let percentage =
        Math.round((this.props.data[index].value * 1000) / this.state.dataSum) /
          10 +
        "%";
          ...
          >
            ...
            >
              ...
              <Text style={styles.tooltipTitle}>
                {this.state.labels[index] ? this.state.labels[index] : valueStr}
              </Text>
            </View>
      ...
      return (
        ...
        >
          ...
          >
            ...
            <Text style={styles.tooltipValue}>
              {(this.props.data[index].value * 100) / this.state.dataSum + "%"}
            </Text>
          </View>
        </View>
      );
  ...
  drawT() {
    ...
    for (let i = 0; i < this.state.pieSize.length; i++) {
      pies.push(
        ...
        >
          {this.drawPie(
            this.state.pieSize[i],
            this.state.colors[this.state.pieIndex[i]],
            true,
            i
          )}
        </View>
