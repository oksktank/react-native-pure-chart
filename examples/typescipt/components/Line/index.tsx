...
type LineChartProps = {
  ...
  data: any;
  primaryColor: string;
  xAxisGridLineColor: string;
  onPress: (evt: number) => void;
  selectedColor: string;
  hidePoints: boolean;
  customValueRenderer: (index: number, point: any) => React.ReactNode;
  ...
  minValue: number;
  xAxisColor: string;
  showEvenNumberXaxisLabel: boolean;
};
...
class LineChart extends React.Component<LineChartProps, LineChartState> {
  ...
  drawCoordinate(
    index: number,
    start: { gap: number; ratioY: number },
    end: { gap: number; ratioY: number },
    backgroundColor: string,
    lineStyle: { borderColor?: any },
    isBlank: boolean,
    lastCoordinate: boolean,
    seriesIndex: number
  ) {
    let key = "line" + index;
  ...
  drawCoordinates(data: any[], seriesColor: string, seriesIndex: number) {
    let result = [];
    let lineStyle = {
